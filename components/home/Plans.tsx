        ...
        <div className="mt-10 grid grid-cols-2 md:grid-cols-5 gap-4">
          {ITEMS.map((item: any, i: number) => (
            ...
            > <p className="text-xl p-3 px-5 text-center">Plan</p>
              <div className="rounded-b-2xl px-10 py-5 pt-2 bg-white border-t border-lime-400  flex flex-col h-40">
                <p className="Grotesque-font group-hover:scale-110 duration-300 text-4xl font-semibold text-lime-600">
                  {item.plan}
                </p>
                <p className="text-sm">{item.size}</p>
                <button className="flex items-center gap-3 text-sm justify-center absolute w-40 bottom-4 left-[50%] text-white group-hover:shadow-lg duration-300 -translate-x-[50%] p-2 rounded-3xl bg-lime-500 mt-4">Get Started <RiArrowRightLine />
                </button>
              </div>
            </div>
          ))}
