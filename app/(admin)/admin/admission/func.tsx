...
export async function getAsmission(
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      console.error("Failed to get admission:", response.statusText);
      return null; 
    }
  } catch (error) {
...
export async function deleteAdmission(
  ...
  const URL: string = `${ROOT_URL}admission/actions.php?api=${API_KEY}&id=${id}`;
...
const URL: string = `${ROOT_URL}upload.php?api=${API_KEY}&filename=${file}&folder=${folder}`;
...
try {
  ...
  if (response.status === 200) {
    ...
    console.error(`Failed to get file:`, response.statusText);
    return null; 
  }
} catch (error) {
  console.error("Error:", error);
